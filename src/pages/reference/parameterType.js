...
const ParameterType = () => {
  const { state, dispatch, message } = useParameterTypeContext();
  ...
  const deleteItem = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        API.deleteParameterType(item.id)
          .then(() => {
  ...
  const save = () => {
    ...
    if (error.length > 0) {
      ...
      if (state.id === null) {
        ...
        API.putParameterType(state.id, data)
          .then(() => {
            dispatch({ type: "REFRESH" });
            dispatch({ type: "CLEAR" });
            dispatch({ type: "MODAL", data: false });
