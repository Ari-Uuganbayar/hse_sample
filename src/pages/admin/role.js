...
import { useRoleContext } from "src/contexts/admin/roleContext";
...
const Role = () => {
  const { state, dispatch, message } = useRoleContext();
    ...
    API.getRole(item.roleid)
      ...
      .catch((error) => {
        message({
          type: "error",
          error,
          title: "Бүлгийн мэдээлэл татаж чадсангүй",
        });
      });
  };
        ...
        API.postRole(data)
          ...
          .catch((error) => {
            message({
              type: "error",
              error,
              title: "Бүлэг бүртгэж чадсангүй",
            });
          });
      } else {
        API.putRole(state.id, data)
          .then(() => {
            message({ type: "success", title: "Амжилттай хадгалагдлаа" });
            dispatch({ type: "REFRESH" });
            dispatch({ type: "MODAL", data: false });
          ...
          .catch((error) => {
            message({
              type: "error",
              error,
              title: "Бүлэг засварлаж чадсангүй",
            });
          });
      }
    }
